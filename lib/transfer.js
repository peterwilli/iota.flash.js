...
function prepare(settlement, deposits, fromIndex, destinations) {
  ...
  settlement.map((s, i) => {
    if (i != fromIndex) {
      ...
      const totalDeposit = deposits.filter((e, i) => i != fromIndex).reduce((acc, s) => acc + s, 0);
      var stake = 0;
      if (totalDeposit > 0) {
        stake = total * deposits[i] / totalDeposit;
      }
      if (current) {
        current.value += stake;
        current.obsoleteTag = ''
...
function compose(balance, deposit, outputs, root, remainder, history, transfers, close) {
  ...
  multisigs.slice(0, multisigs.length - 1).map((multisig, i) => {
    ...
    IOTACrypto.multisig.initiateTransfer(input, remainder.address, transfers, (err, success) => {
...
function applyTransfers(root, deposit, outputs, remainder, history, transfers) {
  if (transfers.filter(transfer => transfer.filter(tx => tx.value < 0).filter(tx => !IOTACrypto.utils.validateSignatures(transfer, tx.address)).length != 0).length != 0) {
...
function sign(root, seed, bundles) {
  ...
  return helpers.deepClone(bundles).map((bundle, i) => {
    ...
    bundle.filter(tx => tx.address == multisig.address).slice(0, multisig.signingIndex).map(tx => {
      if (IOTACrypto.utils.inputValidator.isNinesTrytes(tx.signatureMessageFragment)) {
        tx.signatureMessageFragment = tx.signatureMessageFragment.replace(/^9/, 'A');
      }
    });
    ...
    IOTACrypto.multisig.addSignature(bundle, multisig.address, IOTACrypto.multisig.getKey(seed, multisig.index, multisig.security), (err, suc) => {
      sigs = {
        ...
        signatureFragments: suc.filter(tx => tx.address == multisig.address).map(tx => tx.signatureMessageFragment).slice(multisig.signingIndex, multisig.signingIndex + multisig.security)
      }
    })
    return sigs
  });
...
function appliedSignatures(bundles, signatures) {
  return helpers.deepClone(bundles).map((bundle, i) => {
    ...
    if (userSignature) {
      ...
      bundle.filter(tx => tx.address == addy).slice(userSignature.index, userSignature.index + userSignature.signatureFragments.length).map((tx, j) => tx.signatureMessageFragment = userSignature.signatureFragments[j]);
      // add signature
    }
    return bundle;
  });
...
function getDiff(root, remainder, history, bundles) {
  ...
  let previousTransfer = history.length == 0
    ? []
  ...
  const newRemainder = lastTransfer.filter(tx => tx.address == remainder.address).map(tx => tx.value).reduce((acc, v) => acc + v, 0)
  if (newRemainder.value > previousRemainder.value) {
    throw new Error(TransferErrors.REMAINDER_INCREASED);
  }
  const newCopy = helpers.deepClone(lastTransfer.filter(tx => tx.value > 0).map(tx => Object({address: tx.address, value: tx.value}))).filter(tx => tx.address !== remainder.address)
